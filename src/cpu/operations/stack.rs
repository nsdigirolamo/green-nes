...
pub fn pha(state: &mut State) {
    state.buses.write(state.buses.addr, state.registers.a);
}
...
pub fn plp(state: &mut State) {
    ...
    let masked_stack_status = state.buses.read(state.buses.addr) & 0b_1100_1111;
...
pub fn pla(state: &mut State) {
    let result = state.buses.read(state.buses.addr);
