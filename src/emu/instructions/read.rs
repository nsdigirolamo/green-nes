use crate::emu::{
    ...
    half_cycles::{
        ...
        get_indirect_x_indexed_low_address_byte, get_indirect_y_indexed_address_with_carry,
        get_indirect_zero_page_high_address_byte, get_indirect_zero_page_low_address_byte,
        get_pc_address, read_data, read_high_base_address_byte, read_high_effective_address_byte,
        read_low_base_address_byte, read_low_effective_address_byte,
        read_low_indirect_address_byte,
    },
...
impl Instruction for Read {
    fn get_cycles(&self, operation: HalfCycle) -> Vec<Cycle> {
        match self {
            Read::Immediate => vec![[get_pc_address, operation]],
            Read::ZeroPage => vec![
                FETCH_LOW_EFFECTIVE_ADDRESS_BYTE,
            ...
            Read::IndirectY => vec![
                ...
                [get_indirect_y_indexed_address_with_carry, operation],
            ],
            Read::AbsoluteX => panic!("read absolute x not implemented"),
            Read::AbsoluteY => panic!("read absolute y not implemented"),
            Read::ZeroPageX => vec![
                FETCH_LOW_BASE_ADDRESS_BYTE,
                [get_base_zero_page_address, read_data],
